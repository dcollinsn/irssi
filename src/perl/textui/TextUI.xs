...
static void perl_text_buffer_view_fill_hash(HV *hv, TEXT_BUFFER_VIEW_REC *view)
{
	...
	(void) hv_store(hv, "bottom_startline", 16, buffer_line_bless(view->buffer, view->bottom_startline), 0);
...
{
	(void) hv_store(hv, "info", 4, plain_bless(&line->line->info, "Irssi::TextUI::LineInfo"), 0);
}
...
CODE:
	...
	gui_printtext_after_time(&dest, prev->line, text2, time);
	g_free(text);
	g_free(text2);
...
CODE:
	format_create_dest(&dest, NULL, NULL, level, window);
	gui_printtext_after_time(&dest, prev->line, str, time);

Irssi::TextUI::Line
last_line_insert(window)
	Irssi::UI::Window window
CODE:
	RETVAL = perl_wrap_buffer_line(WINDOW_GUI(window)->view->buffer, WINDOW_GUI(window)->insert_after);
OUTPUT:
	RETVAL
