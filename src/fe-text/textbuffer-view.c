                            ...
                            unsigned int *bg24)
{
	...
	case FORMAT_COLOR_24:
		unformat_24bit_line_color((const unsigned char **) ptr, 0, color, fg24, bg24);
		break;
#endif
	default:
		...
		if (**ptr != FORMAT_COLOR_NOCHANGE) {
			if (**ptr == (unsigned char) 0xff) {
				...
				*color |= (((unsigned char) **ptr - '0') & 0xf) << BG_SHIFT;
			}
		}
	}
	if (**ptr == '\0')
...
view_update_line_cache(TEXT_BUFFER_VIEW_REC *view, LINE_REC *line)
{
	...
	if (line_text != NULL) {
		for (ptr = (unsigned char *) line_text;;) {
			...
			if (xpos + char_width > view->width || newline) {
				xpos =
				...
				if (newline) {
					xpos = 0;
				} else if (last_space > indent_pos && last_space > 10) {
				...
				sub->start = ptr;
				sub->indent = xpos;
				sub->indent_func = newline ? NULL : indent_func;
				sub->color = color;
#ifdef TERM_TRUECOLOR
				sub->fg24 = fg24;
				sub->bg24 = bg24;
